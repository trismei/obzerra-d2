...
if not st.session_state.models_trained:
    try:
        ...
        if os.path.exists(training_file):
            with st.spinner('🤖 Training ML models with real fraud data...'):
                success = ml_manager.train_from_real_data(training_file)
                if success:
                    st.session_state.models_trained = True
                    st.toast('✅ ML models trained successfully!', icon='🎯')
...
NAV_TABS = ["📊 Dashboard", "🔍 Single Claim", "📤 Batch Upload", "📋 History"]
...
# Batch Upload Tab
elif st.session_state.active_tab == "📤 Batch Upload":
        ...
        try:
            ...
            if st.button("🔍 Analyze Claims for Fraud", type="primary"):
                with st.spinner("Processing claims through fraud detection engine..."):
                    ...
                    if has_actual_fraud and fraud_mapping is not None:
                        ...
                        if len(valid_rows) > 0:
                            ...
                            if pred_col:
                                ...
                                st.warning("⚠️ Could not find prediction column for accuracy calculation")
                    ...
                    success_msg = f"✅ Analysis complete! Processed {len(results)} claims in {runtime:.2f} seconds."
            ...
            if 'batch_results' in st.session_state and st.session_state.batch_results is not None:
                ...
                if len(filtered_results) > 0:
                    ...
                    for idx, row in filtered_results.iterrows():
                        ...
                        rules_html = ''.join(rule_icons) if rule_icons else '<div style="color: #10b981; font-style: italic;">✅ No suspicious patterns detected - claim appears normal</div>'
                        
                        # Build HTML in parts to avoid f-string issues
                        card_header = f'<div class="claim-card"><div class="claim-header"><div class="claim-id">📋 {safe_claim_id}</div>'
                        ...
                        card_details = '<div class="claim-details"><div class="detail-section"><div class="detail-title">🤖 AI Assessment</div>'
                        ...
                        if 'actual_fraud' in row and pd.notna(row['actual_fraud']):
                            ...
                            prediction_match = '✅ Match' if actual_fraud == row['fraud_prediction'] else '❌ Mismatch'
                        ...
                        card_details += f'<div class="detail-section"><div class="detail-title">🔍 Why This Result?</div><div class="detail-content">{rules_html}</div></div></div></div>'
...
# Single Claim Tab
elif st.session_state.active_tab == "🔍 Single Claim":
